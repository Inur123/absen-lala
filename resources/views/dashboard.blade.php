...
<div class="flex flex-col gap-6">
  ...
  <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
    ...
    <div class="rounded-lg border bg-card p-6 text-card-foreground shadow-sm">
      ...
      <div class="mt-3">
        <p class="text-2xl font-bold" x-text="pesertaStats.total"></p>
        <p class="text-xs text-muted-foreground">
          <span class="text-green-500 dark:text-green-400">+<span x-text="pesertaStats.newThisWeek"></span></span> peserta baru minggu ini
        </p>
      </div>
      <div class="mt-4 flex items-center justify-between text-xs text-muted-foreground">
        <div class="flex items-center gap-1">
          <span class="h-2 w-2 rounded-full bg-blue-500"></span>
          <span>Laki-laki: <span x-text="pesertaStats.lakilaki"></span></span>
        </div>
        <div class="flex items-center gap-1">
          <span class="h-2 w-2 rounded-full bg-pink-500"></span>
          <span>Perempuan: <span x-text="pesertaStats.perempuan"></span></span>
        </div>
      </div>
    </div>
    ...
    <div class="rounded-lg border bg-card p-6 text-card-foreground shadow-sm">
      ...
      <div class="mt-3">
        ...
        <p class="text-xs text-muted-foreground">
          <span class="text-green-500 dark:text-green-400"><span x-text="absensiStats.averageAttendance"></span>%</span> rata-rata kehadiran
        </p>
      </div>
          ...
          <div class="flex items-center justify-between rounded-lg border p-4">
            ...
            <div class="flex items-center gap-4">
              <div class="text-right">
                <p class="text-sm font-medium" x-text="`${item.pesertaHadir}/${item.totalPeserta}`"></p>
                <p class="text-xs text-muted-foreground" x-text="`${getAttendancePercentage(item)}% Hadir`"></p>
              </div>
              <a
                href=""
                class="rounded-full bg-primary/10 p-2 text-primary hover:bg-primary/20">
    ...
    <div class="rounded-lg border bg-card p-6 text-card-foreground shadow-sm">
      ...
      <div class="mt-4 flex justify-center gap-6">
        <div class="flex items-center gap-2">
          ...
          <span class="text-sm">Laki-laki: <span x-text="pesertaStats.lakilaki"></span></span>
        </div>
        <div class="flex items-center gap-2">
          <span class="h-3 w-3 rounded-full bg-pink-500"></span>
          <span class="text-sm">Perempuan: <span x-text="pesertaStats.perempuan"></span></span>
        </div>
      </div>
    </div>
